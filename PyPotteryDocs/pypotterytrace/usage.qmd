...
- Click **🔄 Refresh** to update the list if you've made changes externally.
...
- **⚡ Tiny** (~40MB): Very fast, good for older computers or quick checks.
- **⭐ Small** (~180MB): **Recommended.** The best balance between speed and accuracy for most users.
- **🎯 Base+** (~230MB): Higher accuracy but slower.
- **🚀 Large** (~900MB): Best possible accuracy, but requires significant memory and processing time.
...
- **👆 Point Mode (Default)**: Click on the object you want to segment.
    - **➕ Positive**: Click to ADD to the mask (Green points).
    - **➖ Negative**: Click to REMOVE from the mask (Red points).
...
- **⬡ Manual Polygon**: Manually click points to define the exact shape if AI fails.
...
- **🏺 Profile**: The main vessel profile.
- **🎯 Application**: Applied decorations over the profile.
- **🪢 Handle**: Handles or lugs.
- **👁️ Prospectus**: Frontal view.
- **🎨 Decoration**: Surface painting or incision.
- **🏃 Running element**: Continuous patterns over the profile (see example).
- **📌 Detail**: Specific areas of interest.
...
- **✏️ Edit Mask**: Convert an AI-generated mask into a manual polygon for fine-tuning.
...
- **Toolbar**:
    ...
    - **🔍+ / 🔍-**: Zoom in and out.
    - **⟲ Reset View**: Fit image to screen.
    - **↶ Undo**: Undo the last action.

### 👉 Right Sidebar: Segments & Export

- **📋 Segments**: Lists all created masks for the current image. You can remove unwanted segments here.
- **📤 Export**:

    - **🎨 Generate SVG**: Converts your segmentations into a vector file and moves you to the SVG Editor.
    - **🔧 DEBUG Export**: Quick export for testing purposes.
...
Also, you can use the **➕ Positive** and **➖ Negative** buttons to refine the mask. In other words, the positive button will add the selected area to the mask, while the negative button will remove it.

:::

![Defining the vessel profile with a single click.](imgs\2.png)

We can scroll down the left sidebar to the **🏷️ Category** tab. Here we confirm the label as **🏺 Profile**.

![Selecting the Profile category.](imgs\3.png)

The profile will turn **red**, and the segmented layer will appear in the **📋 Segments** list on the right sidebar.

::: {.callout-note}
## Mistake?
If the generated mask is completely wrong, you can delete it by clicking the **Trash Icon 🗑️** next to the segment in the list.
:::

![The profile is highlighted in red.](imgs\4.png)

Now, let's segment the **prospectus** (frontal view). First, switch the category to **👁️ Prospectus** in the **🏷️ Category** tab.

![Switching to the Prospectus category.](imgs\5.png)

For this shape, we can use the **▭ Box Mode** to segment the prospectus efficiently.
...
Clicking on **➕ Add Segment** confirms the selection. The prospectus will turn **blue** and appear in the right sidebar list.

![The prospectus segment is added to the list.](imgs\8.png)

Next, click on **⨁ Rotational Center** to define the vessel's axis of rotation.

![Setting the rotational center.](imgs\9.png)

Finally, click on **🎨 Generate SVG** to create the vector file and proceed to the editor.
...
We also insert the rotational center using **⨁ Rotational Center**.

![Defining the rotational center.](imgs\12.png)

Let's try to generate the SVG by clicking **🎨 Generate SVG**!
...
At this point, we can go back to the **✂️ Segmentation** tab and try to segment the profile more accurately. Sometimes this works, but other times it is necessary to manually edit the profile.

After deleting the incorrect segment, we recreate the mask for the profile, but in this case, we must click on **✏️ Edit Mask** to modify the mask manually.

:::{.callout-tip}
## Create a Manual Mask

You can also create a manual mask from scratch by clicking on **⬡ Manual Polygon**.
...
Once satisfied with my manual modification, I can click on **✅ Done Editing** and then on **➕ Add Segment** to add the mask manually. In fact, on the right sidebar, a badge indicates that our mask has been manually modified.

![Completing the manual edit.](imgs\17.png)

Now let's create the handle mask using **⬡ Manual Polygon**:

![Segmenting the handle separately.](imgs\18.png)

After clicking on **➕ Add Segment**, the handle mask will appear in the segment list, and we are ready to generate the SVG.
...
## Tools
- **👁️ View Mode**: Pan and zoom around the SVG. Drag images to reposition them.
- **👆 Select Mode**: Click points to select them. Shift+Click for multiple. Ctrl+Drag to move them.
- **➕ Add Point**: Click anywhere on a path to insert a new control point.
- **🗑️ Delete Mode**: Click points to remove them.

## Layers & Export
- **📋 Layers**: Manage the different parts of your drawing. You can add the original image as a background layer for reference using **📷 Add Current Image**.
- **💾 Save Modified SVG**: Save your changes.


## A Basic Example

Continuing with our simple vessel example, let's see how to use the **✏️ SVG Editor** to modify the drawing.

![Editing the SVG.](imgs\19.png)

In **👁️ View Mode**, we can pan and zoom around the SVG using the middle mouse button or the scroll wheel.
...
:::{.callout-tip}
## Managing Layers
To manage the various raster masks, you can use the **📋 Layers** panel. Uncheck a raster layer to hide it. You can also adjust its opacity using the slider.
:::

However, while viewing the SVG, you might fear that the vectorization is inaccurate. Fortunately, you can click **📷 Add Current Image** to overlay the original image in the background as a reference.

![Adding the current image as reference.](imgs\21.png)

Everything seems correct. I can remove the reference image from the layer list using the **Trash Icon 🗑️**.

To modify a point, switch to **👆 Select Mode** and click on it.
...
The **➕ Add Point** mode allows you to click anywhere on a path to insert a new control point.

Conversely, the **🗑️ Delete Mode** allows you to click on any point to remove it.

Once you have finished your edits, click **💾 Save Modified SVG** to save your changes.
...
Click **📤 Export Files** to process all vectorized data in your project and download the results.