...
Each card shows the name, the description, how many images were **Uploaded** and how many are **Vectorized**, and the creation and modification dates. Click a card to open the project: the app takes you to the **Setup** tab, and the project name appears in the header (click it to come back to this tab). **Refresh** reloads the list if you changed something outside the app. **Delete** removes the project and everything in it (images, masks, vectors and exports) after a confirmation, and cannot be undone. If you reload the page, the project you had open is reopened.
...
**Moving around the image**: scroll the mouse wheel (or press `+` and `-`) to zoom, drag with the middle mouse button or with `Ctrl` + left button to pan, and use the toolbar under the image to zoom in and out, to **reset the view** and to undo.
...
2. **Export Options**:
    ...
    - **Download ZIP Archive**: bundles everything into one file. Keep it checked: it is how the files reach you.
    - **Category Subfolders**: puts each drawing in a folder named after its main category.
3. Click **Export All Vectorized Files**. A progress bar follows the work, and the ZIP is downloaded by your browser. A window then reports how many files were created.
