...
Before installing PyPotteryTrace, make sure you have:
...
Automated installation is the quickest and easiest way to get PyPotteryTrace running. The provided scripts will:
...
Double click on the `PyPotteryTrace_WIN.bat` file.
...
```bash
chmod +x PyPotteryTrace_UNIX.sh
```

3. Run the script:

```bash
./PyPotteryTrace_UNIX.sh
```
...
3. Opening your browser at `http://localhost:5004` (default port for Trace) should display the PyPotteryTrace interface
