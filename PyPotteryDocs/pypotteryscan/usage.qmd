...
The PyPotteryScan interface is divided into eight main sections:
...
<iframe src="animations/create_project.html" style="width: 100%; height: 450px; border: none; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin: 20px 0;"></iframe>
...
4. Click **"Create"**.
You can create a new project by clicking on **➕ New Project** or **➕ Create First Project** if you don't have any projects yet (@fig-create_project)
...
Clicking on **Start Annotation →** will start the scanning process and take you to the **Annotate** tab.
...
```
project_id/
├── original_images/    # Source scans
├── thumbnails/         # Cached thumbnails
├── annotations/        # JSON annotation files
├── cropped_drawings/   # Intermediate crops
├── cleaned_drawings/   # Cleaned images
├── ocr_results/        # OCR JSON data
├── exports/            # Export history
├── fewshot_examples/   # Saved training examples
...
The **💾 Save Metadata** button allows you to save the inserted information.

Below, there are summary boxes for the created annotations.

The **Finish & Process OCR →** button allows you to save the annotations and proceed to the **Process OCR** tab.
...
To delete a text annotation, you can click on the **❌** button next to the annotation in the list (@fig-add_text).
...
Clicking on **Finish & Process OCR →** will save the annotations and move to the **Process OCR** tab.
...
On first run, the user is asked to choose an OCR engine to download:

- **OlmOCR-FP4**: heavier vision-language model, requires an NVIDIA GPU with CUDA
- **GLM-OCR**: lightweight vision-language model (0.9B params), works on CPU or GPU
- **No OCR model**: skip the download and move to manual text entry

![](imgs\10.png){#fig-process_ocr}

Clicking on **🚀 Start OCR Processing** will start the text recognition process.
...
Clicking on **Continue to Clean Drawings →** will take you to the **Clean Drawings** tab.
...
Click on the navigation button, as well as on **✓ Mark as Clean**, or click on the preview in the gallery to save the cleaned image and move to the next drawing.

A green checkmark will indicate that the image has been manually reviewed (@fig-cleaned_image).

![](imgs\14.png){#fig-cleaned_image}

Click on **Continue to Review Texts →** to save the cleaned images and proceed to the **Review Texts** tab.
...
Click on **Continue to Export →** to save changes and proceed to the **Export** tab.
...
```
[Selected Folder]/
├── images/                                 # Folder containing cropped drawings
│   ├── {prefix}_001.jpg                    # Individual drawing images
│   ├── {prefix}_002.jpg
│   └── ...
├── {prefix}_data_{timestamp}.xlsx          # Excel file with Metadata and OCR results
└── {prefix}_ml_training_{timestamp}.xlsx   # Excel file with Bounding Box data for ML
```
...
<iframe src="animations/fewshot.html" style="width: 100%; height: 450px; border: none; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin: 20px 0;"></iframe>
...
The **📂 Select Files** button allows you to select the `.xlsx` file exported from the **Export** tab.
...
Clicking on **➕ Add Example** will allow you to add an example of association between text and fields (@fig-fewshot_add_example).

![](imgs\20.png){#fig-fewshot_add_example}

Clicking on **🚀 Run Parsing** will start the parsing process. A progress bar will show the advancement of the process. Once processing is finished, a dialog window will allow you to save the exported file in `.xlsx` format (@fig-fewshot_run_parsing).

![](imgs\21.png){#fig-fewshot_run_parsing}

## Load a Project

If you want to load an existing project, you can do so from the initial screen by clicking on **📂 Load Project** (@fig-load_project).
...
The **🗑️** (trash can) icon next to each project allows you to delete the selected project.
...
# Straighten Tool and Keyboard Shortcuts

## Straighten Tool

In the **Clean Drawings** tab, tilted drawings can be straightened before (or after) erasing text:

- **Precision slider**: rotate from -45.0° to +45.0° in 0.1° steps, with a live angle readout.
- **Micro-adjustment buttons**: `-0.5°`, `-0.1°`, `+0.1°`, `+0.5°`, plus `0° Reset` and `-90°` / `+90°` rotations.
- **Alignment grid**: a semi-transparent horizontal and vertical grid with a central crosshair helps you level the vessel's rim or base.
- **Fixed canvas size**: the canvas keeps stable dimensions while you rotate, so the drawing never shrinks or expands.
- **Auto-Fit**: one click trims the excess blank margins around the drawing.

## Keyboard Shortcuts

| Tab | Key | Action |
|---|---|---|
| Annotate | `D` / `T` | Drawing tool / Text tool |
| Annotate | `E` | Eraser |
| Annotate | `←` / `→` | Previous / next image |
| Annotate | `Ctrl+S` | Save current annotations |
| Clean | `E` | Toggle eraser mode |
| Clean | `G` | Toggle alignment grid |
| Clean | `R` | Reset straighten rotation to 0.0° |
| Clean | `[` / `]` | Decrease / increase eraser size |
| Clean | `+` / `-` | Zoom in / out |
| Clean | `M` | Mark current drawing as clean |
| Clean | `Ctrl+Z` | Undo last stroke or rotation |
| Clean | `←` / `→` | Previous / next drawing |
