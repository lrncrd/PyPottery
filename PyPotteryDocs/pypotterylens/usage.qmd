...
Select a polygon in the list and press `Delete` to remove it.
...
Use **Polygon** (`P`) to outline the inner vessel by hand. **Each polygon becomes its own card** when you extract, so the inner vessel gets its own image and its own row in the table.
...
- The cards, renamed `OSA_2024_1.png`, `OSA_2024_2.png`, ... in the order of their file names, **leaving out the cards you excluded**. If you post-processed, they are the cards from `cards_modified/`; otherwise, the plain extracted cards. Each image carries its resolution (300 DPI) in its own metadata.
